...
const UserDetailScreen = ({ navigation, route }: any) => {
  ...
  const constituency = user.target?.name || user.village || 'General Node';

  // Deterministic random selection based on user ID
  ...
  const portraitSource = user.image_url
    ? { uri: mediaService.getFileUrl(user.image_url) }
    : getRandomLocalImage(user.id || 0, 3);
  ...
  const checkFollowStatus = async () => {
    try {
      const status = await candidateService.getFollowStatus(user.id);
  ...
  const toggleFollow = async () => {
    ...
    try {
      ...
      if (previousState) {
        ...
        result = await candidateService.unfollowCandidate(user.id);
      } else {
        ...
        return (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <Text style={styles.cardTitle}>About Member</Text>
            </View>
            <View style={styles.cardBody}>
              ...
              <View style={styles.infoGrid}>
                <View style={styles.infoBox}>
                  ...
                  <Text style={styles.infoValue}>{user.date_of_birth || "N/A"}</Text>
                </View>
                <View style={styles.infoBox}>
              ...
              <View style={styles.registryBox}>
                ...
                <Text style={styles.infoValue}>{user.parent_name || "N/A"}</Text>
              </View>
            </View>
          </View>
        );
      case 'registry':
        return (
          <View style={styles.card}>
            ...
            <View style={styles.cardBody}>
              <DetailRow label="Designation" value={user.designation || "Member"} />
              <DetailRow label="Official Role" value={user.role || "N/A"} />
              
              <View style={styles.divider} />

              <DetailRow label="State" value={user.state || "N/A"} />
              <DetailRow label="District" value={user.district || "N/A"} />
              <DetailRow label="Village / Area" value={user.village || "N/A"} />
              <DetailRow label="Pincode" value={user.pincode || "N/A"} />
            </View>
          </View>
        );
      default:
      ...
      >
          ...
          <View style={styles.portraitWrapper}>
            {user.image_url ? (
              ...
              <View style={[styles.heroPortrait, { backgroundColor: COLORS.primaryContainer, justifyContent: 'center', alignItems: 'center' }]}>
                <MaterialIcons name="person" size={ms(64)} color="#fff" />
              </View>
            )}
          </View>
          ...
          <View style={styles.profileInfo}>
            <View style={styles.badgeRow}>
              <View style={styles.statusBadge}>
                ...
                <Text style={styles.statusBadgeText}>{user.is_verified ? 'VERIFIED' : 'PENDING'}</Text>
              </View>
              {user.is_candidate && (
                <View style={[styles.statusBadge, { backgroundColor: COLORS.primary }]}>
                  <MaterialIcons name="stars" size={ms(12)} color="#fff" />
                  <Text style={styles.statusBadgeText}>CANDIDATE</Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.metaRow}>
              ...
              <View style={styles.metaItem}>
                <MaterialIcons name="badge" size={ms(14)} color={COLORS.onSurfaceVariant} />
        ...
        <View style={styles.contentPadding}>
          ...
          <View style={styles.securityBanner}>
            ...
            <Text style={styles.securityText}>Member profile data is managed within the secure organizational network.</Text>
          </View>
        </View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  followBtn: { backgroundColor: COLORS.primary, width: '80%', paddingVertical: vs(14), borderRadius: ms(8), flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: hs(10) },
  ...
  card: { backgroundColor: '#fff', borderRadius: ms(10), borderWidth: 1, borderColor: COLORS.outlineVariant, padding: hs(20) },
